//
// Subworkflow with functionality specific to the xiaoli-dong/nf-taxflow pipeline
//
...
workflow PIPELINE_INITIALISATION {
    ...
    //sample,fastq_q,fastq_2,long_fastq,fasta_contig
    ...
    reads = channel.fromPath(params.input)
        ...
        .map { row ->
            ...
            [
                ...
                fasta_contig: row.fasta_contig == 'NA' ? null : row.fasta_contig,
            ]
        }
        .filter { it != null }
    ...
    // Create channel for contig fasta only (where long_fastq is not null)
    fasta_contig = reads
        .filter { it.fasta_contig != null }
        .map { row ->
            def meta = [
                id: row.sample,
                single_end: true
            ]
            tuple(meta, file(row.fasta_contig))
        }
